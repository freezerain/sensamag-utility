...
def __parse_row(cur: mariadb.Cursor, row: dict) -> None:
    ...
    if ref_id and ref_name:
        cur.execute("UPDATE textreferences SET Name = ? WHERE Id = ?", (ref_name, ref_id))
    ...
    if lang_id and lang_name:
        cur.execute("SELECT Id, Name FROM localizationlanguages WHERE Id = ?", (lang_id,))
        lang_db = cur.next()[1]
        if lang_name.casefold() != lang_db.casefold():
            raise ValueError("Language name from CSV do not match ones in DB!"
    ...
    # If language name provided but not ID -> get ID from DB
        ...
        cur.execute("SELECT Id, Name FROM localizationlanguages WHERE Name = ?", (lang_name,))
        result = cur.next()
    ...
    if cont_id:
        ...
        cur.execute("SELECT Id, Text, TextReferences_Id, Language_Id "
                    "FROM textcontents WHERE Id = ?", (cont_id,))
        ...
        cur.execute("UPDATE textcontents SET Text = ?, TextReferences_Id = ?, Language_Id = ? "
                    "WHERE Id = ?", (cont_text if cont_text else content_result[1],
    ...
    # If text provided but not id -> create new content entry with provided text
        ...
        if not lang_id or not ref_id:
            raise Exception(f"To create new text content entry - "
                            f"lang_id and ref_id must be provided!"
                            f"\nProvided: lang_id: {lang_id}, ref_id: {ref_id}.")
        cur.execute("INSERT INTO textcontents (name, text, Language_Id, TextReferences_Id) "
                    "VALUES (?, ?, ?, ?)", (ref_name, cont_text, ref_id, lang_id))
